...
func TestAntiWindupController_PIDUpdate(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		assert.Assert(t, Abs(tt.expectedState.ControlError-c.State.ControlError) < deltaTest)
		assert.Assert(t, Abs(tt.expectedState.ControlSignal-c.State.ControlSignal) < deltaTest)
		deltaI := Abs(tt.expectedState.ControlSignal - c.State.ControlErrorIntegral*c.Config.IntegralGain)
		assert.Assert(t, deltaI < deltaTest)
		assert.Assert(t, Abs(tt.expectedState.ControlErrorIntegrand-c.State.ControlErrorIntegrand) < deltaTest)
		deltaD := Abs(tt.expectedState.ControlErrorDerivative - c.State.ControlErrorDerivative*c.Config.DerivativeGain)
...
func TestAntiWindupPID_FFUpdate(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		assert.Assert(t, Abs(tt.expectedState.ControlError-c.State.ControlError) < deltaTest)
		assert.Assert(t, Abs(tt.expectedState.ControlSignal-c.State.ControlSignal) < deltaTest)
		assert.Assert(t, Abs(tt.expectedState.ControlErrorIntegral-c.State.ControlErrorIntegral) < deltaTest)
	}
}
...
func TestAntiWindupPID_NaN(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		// Then
		assert.Assert(t, !IsNaN(c.State.UnsaturatedControlSignal))
		assert.Assert(t, !IsNaN(c.State.ControlSignal))
		assert.Assert(t, !IsNaN(c.State.ControlErrorIntegral))
	}
}
